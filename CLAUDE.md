...
2. **数据库解密**: `tgreader decrypt` → 读取 `all_keys.json` → 逐页 AES-256-CBC 解密 → 验证 HMAC → 输出到 `decrypted/`
3. **消息查询**: `tgreader messages` → 打开解密后的 `message/message_0.db` → 通过 MD5(username) 定位 `Msg_<hash>` 表 → 解析 tgid 前缀为联系人昵称 → 展示
